...
public class Comment {
	...
	@JoinColumn(name = "user_id") // FK: comment.user_id → site_user.id
	private SiteUser author; // 작성자

	@ManyToOne // 다:1 — 여러 댓글이 하나의 게시글(board)에 속함
	@JoinColumn(name = "board_id") // FK: comment.board_id → board.id
	...
	@JoinTable(name = "Comment_Voter", // 조인 테이블 이름
			joinColumns = @JoinColumn(name = "comment_id"), // 현재 엔티티(Comment)를 참조하는 FK
			inverseJoinColumns = @JoinColumn(name = "user_id") // 반대 엔티티(SiteUser)를 참조하는 FK
	)
